...
- `例文`
  ...
  - `この例文の単語` の各チップをタップすると、追加情報とチェック欄を展開表示
  - 単語チップは通常時はコンパクト表示で、展開時のみ横に広がる
  - チェックを入れるとそのチップの背景色を反転し、折りたたんだ後も状態を維持
- `単語`
  - section に関係なく、チェック済み単語だけを表示
  - 単語番号が若い順に表示
  - 最初は `単語 + 追加情報 + チェック欄` を表示
  - 1 回タップすると、その単語が使われている `英文 + 日本語訳` を表示
  - もう 1 回タップすると、その単語のチェックを外して次の単語へ進む
  - すべて外すと `チェックされた単語はありません` を表示
- `英語音声`
  - 1 文ごとに 5 連結音声を再生
  - 最初から `例文 + 日本語訳 + 単語` を表示
  - 例文欄タップで表示を段階的に切り替え
...
`この例文の単語` の各チップは、タップするたびに通常表示と展開表示を切り替えます。

通常時のチップは 1 画面内に収まりやすいコンパクト表示で、展開時のみ横幅を広げて追加情報とチェック欄を表示します。
...
広い画面では `単語 | 追加情報 | チェック欄` の 3 列で表示し、狭い画面では自動で縦方向に折り返します。

チェック欄にチェックを入れると、そのチップの背景色を反転します。  
チップをもう一度タップして通常表示に戻しても、背景反転はチェックを外すまで維持されます。
背景反転時も、追加情報の見出しを含めて読める配色へ自動で切り替えます。

追加情報は `vocab.extraInfo` として後から追加できる前提です。  
改行と空行は展開表示で保持され、空行ごとに段落化されます。`【動】` `【名】` などの見出し行は強調表示します。  
追加情報内の `/.../` 形式の発音記号は自動で検出し、通常本文とは別の色とフォントで表示します。  
背景反転時も、発音記号は通常本文色に埋もれないよう専用色を維持します。  
チェック状態は今後、単語テストへの反映に利用できるよう保存します。

## 単語タブ仕様

`単語` タブでは、例文タブでチェックした単語だけを section 横断で集めて表示します。

1. `vid` の若い順に単語を表示
2. 初期表示は `単語 + 追加情報 + チェック欄`
3. タップすると、その単語を含む `英文 + 日本語訳` を表示
4. もう一度タップするとチェックを外し、次の単語へ進む
5. チェック済み単語がなくなると `チェックされた単語はありません` を表示

単語タブ内のチェック欄を直接外した場合も、その単語は一覧から外れます。
